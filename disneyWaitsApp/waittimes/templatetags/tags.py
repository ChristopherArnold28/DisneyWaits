...
def return_map_url(search_string, display_name, is_ride = 0):
    ...
    if is_ride == 1:
        url = "https://maps.googleapis.com/maps/api/staticmap?center="+str(search_string) + "&zoom=16&size=400x400&markers=color:green|" + str(search_string)+"&maptype=hybrid&key="+ str(apikey)
    else:
        url = "https://maps.googleapis.com/maps/api/staticmap?center="+str(search_string) + "&zoom=15&size=400x400&markers=color:red|" + str(search_string)+"&maptype=hybrid&key="+str(apikey)
    return {'url':url}
...
@register.simple_tag
def check_notifications(rideid, current_user_id):
    notifications = UserNotifications.objects.filter(userid = current_user_id, rideid = rideid)
